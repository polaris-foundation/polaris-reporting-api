...
K8S_DHOS_REPORTING_API_NAMESPACES=dhos-reporting-api
kubectl get ns ${K8S_DHOS_REPORTING_API_NAMESPACES} > /dev/null 2>&1

# create our namespace if it doesn't exist
if [[ "$?" -ne "0" ]]; then
  kubectl create ns ${K8S_DHOS_REPORTING_API_NAMESPACES}
fi
...
helm upgrade --install --namespace ${K8S_DHOS_REPORTING_API_NAMESPACES} dhos-reporting-api $(dirname ${BASH_SOURCE})/helm-chart/dhos-reporting-api \
     -f $(dirname ${BASH_SOURCE})/helm-chart/dhos-reporting-api/values.yaml \
     -f $(dirname ${BASH_SOURCE})/helm-chart/dhos-reporting-api/values-az-uk-dev.yaml \
     -f <(sops --decrypt $(dirname ${BASH_SOURCE})/helm-chart/dhos-reporting-api/values-az-uk-dev-secrets-sops.yaml)
